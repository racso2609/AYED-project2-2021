...
void printQueue(PriorityQueue<Client> *menu) {
  if (!menu->isEmpty()) {
    Client temp = menu->head()->info;
    cout << " name: " << temp.name << " pacience: " << temp.pacience
         << " Priority: " << temp.priority << "Start time"
         << temp.entryTime / 60 << ":" << temp.entryTime % 60
         << "exit Time: " << int(temp.exitTime / 60) << ":"
         << int(temp.exitTime % 60) << endl;
    while (!temp.chosenDishes.isEmpty()) {
      cout << temp.chosenDishes.top->info.name << "->"
           << temp.chosenDishes.top->info.time << endl;
...
void processSolution(Client &client, int &actualTime, Menu menus) {
  // while inteads menu decission and put again menus on stack depend prepare
  // time
  string priority = client.priority == 1 ? " VIP " : " ";
  string Toprint = client.name + priority + "Entro al Local";
  cout << "[" << int(client.entryTime / 60) << ':' << int(client.entryTime % 60)
       << "]" << Toprint << endl;
  ...
  while ((file = getchar()) != EOF) {
    ...
    if (char(file) == '+') {
      ...
      for (int i = 0; i < peopleCount; i++) {
        ...
        string toPrint = "[" + to_string(person.exitTime / 60) + " : " +
                         to_string(person.exitTime % 60) + "] " + person.name +
                         " Priority: " + to_string(person.priority) +
                         printDishes(person);
